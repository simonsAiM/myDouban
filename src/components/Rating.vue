<template>
  <div class="rating">
      <span v-if="full === 0" class="no-rate">暂无评分</span>
      <template v-else>
          <span class="rate-star">
            <span v-for="n in full" class="full"></span><span v-for="n in gray" class="gray"></span>
          </span>
          <span class="average">{{rate.average}}</span>
      </template>
  </div>
</template>
<script>
